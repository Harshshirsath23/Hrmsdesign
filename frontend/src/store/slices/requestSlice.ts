...
export const createRequest = createAsyncThunk(
  'requests/create',
  async (payload: Omit<ProfileRequest, 'id' | 'status' | 'createdAt' | 'reviewedBy' | 'reviewedAt'>, { dispatch }) => {
    await new Promise(resolve => setTimeout(resolve, 500));
    ...
    writeRequests(reqs);
    dispatch(addNotification({ type: 'info', message: 'Your request has been sent to admin.' }));
    return newRequest;
  }
);
...
export const reviewRequest = createAsyncThunk(
  'requests/review',
  async ({ requestId, status, reviewer, rejectionComment, adminRemark, employeeId, section, finalData }: { 
    requestId: string, 
    status: RequestStatus, 
    reviewer: string, 
    rejectionComment?: string,
    ...
    if (index >= 0) {
      ...
      if (status === 'approved' && finalData) {
        ...
        dispatch(addNotification({ type: 'success', message: 'Profile update request approved.' }));
      } else if (status === 'rejected') {
        dispatch(addNotification({ type: 'warning', message: 'Profile update request rejected.' }));
      }
