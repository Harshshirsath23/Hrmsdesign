...
import { IMPORTER_TYPES, MASTER_FIELDS } from './constants';
...
export function ImportModal({ isOpen, onClose, onSuccess }: ImportModalProps) {
  ...
  const handleImporterTypeChange = (value: string) => {
    setState((prev) => ({ ...prev, importerType: value }));
  };
  ...
  const proceedToStep2 = () => {
    if (!state.importerType || !state.file) {
      alert('Please select both importer type and file');
      return;
    }
  ...
  const proceedToStep3 = () => {
    if (state.mappings.some((m) => !m.mappedTo)) {
  ...
  const handleSubmit = () => {
    alert('Import submitted successfully!');
    onSuccess?.({
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Import from Excel">
      <div className="space-y-6">
            ...
            <>
              ...
              <div>
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {IMPORTER_TYPES.map((group) => (
                    <div key={group.groupName}>
                      <div className="text-xs font-bold text-text-tertiary px-2 py-1.5 uppercase">
                        {group.groupName}
                      </div>
                      {group.items.map((item) => (
                        <button
                          key={item.id}
                          onClick={() => handleImporterTypeChange(item.id)}
                          className={`w-full text-left px-3 py-2 rounded transition-all text-sm ${
                            state.importerType === item.id
                              ? 'bg-brand-100 border border-brand-300 text-brand-900 dark:bg-brand-900/30 dark:border-brand-700 dark:text-brand-300'
                              : 'bg-surface-50 border border-surface-200 hover:bg-surface-100 dark:bg-white/3 dark:border-white/10 dark:hover:bg-white/5 text-text-primary'
                          }`}
                        >
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-surface-300 dark:border-white/10 rounded-lg p-6 text-center hover:border-brand-500 transition-colors cursor-pointer">
                  <input
                    type="file"
                    accept=".xlsx,.csv,.xls"
          ...
          {state.step === 2 && (
            <>
              <div className="bg-surface-50 dark:bg-white/3 border border-surface-200 dark:border-white/10 rounded-lg p-4 text-sm">
                <p className="font-medium text-text-primary mb-2">Map Excel Columns to Fields</p>
                <p className="text-text-tertiary">Select the corresponding master field for each column in your Excel file.</p>
              </div>
                ...
                {state.mappings.map((mapping, idx) => (
                  <div key={mapping.id} className="border border-surface-200 dark:border-white/10 rounded-lg p-3 space-y-2">
                    <div className="grid grid-cols-3 gap-2 items-center text-xs">
                      <div>
                        <div className="font-semibold text-text-primary">Excel Column:</div>
                        <div className="text-text-secondary">{mapping.excelField}</div>
                      </div>
                      <div className="text-center text-text-tertiary">→</div>
                      <div>
                        <label className="block text-xs font-semibold text-text-secondary mb-1">Map to Field</label>
                        <select
            ...
            <>
              {state.isValidating ? (
                <div className="text-center py-8 space-y-3">
                  ...
                  <p className="text-sm font-medium text-text-primary">Validating your data...</p>
                </div>
              ) : (
                <div className="space-y-4">
                      ...
                      <div className="bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-800 rounded-lg p-3">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="font-semibold text-red-900 dark:text-red-300">
                            {state.validationErrors.length} Validation Error{state.validationErrors.length !== 1 ? 's' : ''}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="bg-surface-50 dark:bg-white/3 border border-surface-200 dark:border-white/10 rounded-lg p-3 space-y-2 max-h-48 overflow-y-auto">
                        {state.validationErrors.map((error, idx) => (
                          <div key={idx} className="text-xs border-l-2 border-red-500 pl-3 py-1">
                            ...
                            <p className="text-text-tertiary">Value: "{error.value}"</p>
                          </div>
                        ))}
                      </div>

                      <p className="text-xs text-text-tertiary bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-2">
                        💡 Fix the errors above and retry the upload, or proceed to import only valid records.
                      </p>
                    </div>
                  )}
                  ...
                  <div className="bg-surface-50 dark:bg-white/3 border border-surface-200 dark:border-white/10 rounded-lg p-3 space-y-1 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-text-primary">245</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-text-secondary">Valid Records:</span>
                      <span className="font-semibold text-green-600 dark:text-green-400">
                        {245 - state.validationErrors.length}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-text-secondary">Failed Records:</span>
        ...
        <div className="px-6 pb-6 flex gap-3 justify-end border-t border-surface-200 dark:border-white/10 pt-6">
          ...
          {state.step < 3 && (
            <Button variant="primary" iconRight={<ChevronRight className="h-4 w-4" />} onClick={proceedToStep2}>
              Next
            </Button>
          )}
          {state.step === 2 && (
            <Button variant="primary" iconRight={<ChevronRight className="h-4 w-4" />} onClick={proceedToStep3}>
              Validate
            </Button>
          )}
