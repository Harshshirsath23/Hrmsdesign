...
export function BackgroundCheck({ employee }: Props) {
  ...
  const status = employee.backgroundCheck?.verificationStatus || "Pending";
  const config = statusConfig[status] || statusConfig.Pending;
  const StatusIcon = config.icon;

  return (
    <div className="p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <SectionHeader title="Background Check" icon={ShieldCheck} onStart={() => {}} />
      ...
      {!employee.backgroundCheck ? (
        <div className="py-24 text-center bg-card/30 rounded-[3rem] border-2 border-dashed border-border/50 backdrop-blur-sm">
          ...
          <h3 className="text-xl font-black text-foreground mb-2">Background verification not initiated</h3>
          <p className="text-sm font-medium text-muted-foreground max-w-xs mx-auto">Click the button above to start the background verification process for this employee.</p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className={`p-8 rounded-[3rem] border ${config.border} ${config.bg} relative overflow-hidden group`}>
            ...
            <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
              <div className="relative">
                <div className={`w-24 h-24 rounded-full flex items-center justify-center bg-white shadow-2xl ${config.color} transform group-hover:scale-110 transition-transform duration-500`}>
                  <StatusIcon size={48} />
                </div>
                <div className="absolute -bottom-2 -right-2 w-10 h-10 bg-primary text-white rounded-full flex items-center justify-center border-4 border-white shadow-lg">
                  <Edit2 size={16} />
                </div>
              </div>
              ...
              <div className="flex-1 text-center md:text-left space-y-2">
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-4 mt-2">
                  <span className="flex items-center gap-2 text-xs font-bold text-muted-foreground">
                    ...
                    Completed: {formatDate(employee.backgroundCheck.completedOn)}
                  </span>
                  <span className="flex items-center gap-2 text-xs font-bold text-muted-foreground">
                    <Building2 size={14} className="opacity-40" />
                    Agency: {employee.backgroundCheck.agencyName}
                  </span>
                </div>
              </div>

              <div className="flex gap-2">
                <button className="px-6 py-3 bg-white dark:bg-slate-800 border border-border hover:bg-secondary rounded-2xl text-xs font-black transition-all shadow-sm">
                  UPDATE DETAILS
                </button>
                <button className="px-6 py-3 bg-primary text-white hover:bg-primary/90 rounded-2xl text-xs font-black transition-all shadow-lg shadow-primary/20">
                  UPLOAD REPORT
                </button>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-card border border-border rounded-[2.5rem] p-8 space-y-8">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Agency Name</label>
                    <div className="p-4 rounded-2xl bg-secondary/50 border border-border flex items-center gap-3">
                      ...
                      <span className="text-sm font-black">{employee.backgroundCheck.agencyName || "Not Available"}</span>
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Verified By</label>
                    <div className="p-4 rounded-2xl bg-secondary/50 border border-border flex items-center gap-3">
                      <UserCheck size={18} className="text-primary/60" />
                      <span className="text-sm font-black">{employee.backgroundCheck.verifiedBy || "Not Available"}</span>
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Reference Number</label>
                    <div className="p-4 rounded-2xl bg-secondary/50 border border-border flex items-center gap-3">
                      <Hash size={18} className="text-primary/60" />
                      <span className="text-sm font-black">{employee.backgroundCheck.referenceNumber || "Not Available"}</span>
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Completion Date</label>
                    <div className="p-4 rounded-2xl bg-secondary/50 border border-border flex items-center gap-3">
                      <Calendar size={18} className="text-primary/60" />
                      <span className="text-sm font-black">{formatDate(employee.backgroundCheck.completedOn)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="p-6 rounded-[2rem] bg-secondary/30 border border-border">
                    <p className="text-sm font-bold text-muted-foreground leading-relaxed italic">
                      " {employee.backgroundCheck.remarks || "No remarks provided by the verification agency."} "
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6">
              {/* Report Card */}
              {employee.backgroundCheck.reportUrl && (
