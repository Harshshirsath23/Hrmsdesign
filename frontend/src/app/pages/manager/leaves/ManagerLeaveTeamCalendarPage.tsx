...
import { ManagerLeaveStatusBadge } from "../../../components/leaves/manager/ManagerLeaveStatusBadge";
import { useManagerLeaveData } from "./ManagerLeaveDataContext";

export function ManagerLeaveTeamCalendarPage() {
  const { teamApplications } = useManagerLeaveData();
  const today = new Date().toISOString().slice(0, 10);
  const rows = [...teamApplications].filter((a) => a.to_date >= today).sort((a, b) => a.from_date.localeCompare(b.from_date));

  return (
    <div className="space-y-4">
      <header>
        <h1 className="text-lg font-bold tracking-tight text-foreground sm:text-xl">Team calendar</h1>
              ...
              <tbody className="divide-y divide-border">
                {rows.map((a) => (
                  <tr key={a.id} className="transition-colors hover:bg-secondary/50">
                    ...
                    <td className="whitespace-nowrap px-4 py-2.5 text-xs text-muted-foreground">{formatLeaveShortDate(a.from_date)}</td>
                    <td className="whitespace-nowrap px-4 py-2.5 text-xs text-muted-foreground">{formatLeaveShortDate(a.to_date)}</td>
                    <td className="whitespace-nowrap px-4 py-2.5 text-sm font-semibold tabular-nums">{a.total_days}</td>
                    <td className="px-4 py-2.5">
                      <ManagerLeaveStatusBadge status={a.status} />
                    </td>
                  </tr>
                ))}
              </tbody>
