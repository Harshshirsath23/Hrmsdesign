...
const LINKS: { to: string; label: string; icon: LucideIcon; end?: boolean }[] = [
  { to: "/employee/leaves/dashboard", label: "Dashboard", icon: LayoutDashboard },
  { to: "/employee/leaves/apply", label: "Apply Leave", icon: PenLine },
  { to: "/employee/leaves/applications", label: "My Applications", icon: FileStack },
  { to: "/employee/leaves/balance", label: "Leave Balance", icon: Scale },
  { to: "/employee/leaves/holidays", label: "Holiday Calendar", icon: Palmtree },
  { to: "/employee/leaves/team", label: "Team Calendar", icon: Users },
  { to: "/employee/leaves/policy", label: "Leave Policy", icon: ScrollText },
  { to: "/employee/leaves/notifications", label: "Notifications", icon: Bell },
];

export function EmployeeLeaveSidebar({
  collapsed,
  onToggleCollapsed,
}: {
  collapsed: boolean;
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <span
                    className="absolute left-0 top-1/2 hidden h-6 w-0.5 -translate-y-1/2 rounded-r-full bg-foreground lg:block"
                    aria-hidden
                  />
                )}
                <Icon
      ...
      {onToggleCollapsed && (
        <div className="hidden border-t border-border p-2 lg:block">
          <Button
            ...
            size="sm"
            className={cn(
              "h-9 w-full justify-center gap-2 rounded-xl text-xs font-semibold text-muted-foreground",
              collapsed && "px-2",
            )}
