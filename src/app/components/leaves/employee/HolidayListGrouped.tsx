  ...
  return (
    <div className="space-y-4">
      {grouped.map(([month, items]) => (
        <div key={month} className="rounded-3xl border border-border bg-card overflow-hidden">
          <div className="flex items-center justify-between border-b border-border px-5 py-4">
            <p className="text-sm font-semibold text-foreground">{month}</p>
            <span className="rounded-md border border-border bg-secondary px-2.5 py-1 text-[10px] font-semibold text-muted-foreground">
              {items.length}
            </span>
          </div>
