...
export function useRosterCalendar(month: number, year: number, departmentId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const data = await api.fetchRosterCalendarMonthly(month, year, departmentId);
      return { records: mapRosterCalendarToUi(data), meta: data };
    },
  });
}
...
export function useShiftDefinitions() {
  return useQuery({
    queryKey: [...attendanceKeys.shiftMasters(), 'definitions'],
    queryFn: async () => mapShiftMastersToDefinitions(unwrapList(await api.fetchShiftMasters())),
    staleTime: 5 * 60 * 1000,
  });
}
...
export function useAttendanceFilterOptions() {
  return useQuery({
    ...
    queryFn: async () => {
      const [dashboardFilters, matrixDepts, requestDepts, requestTypes] = await Promise.all([
