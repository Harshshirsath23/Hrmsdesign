...
import { FileText, Plus, Search, Edit3, Trash2, Eye, Download, ChevronRight, Tags } from "lucide-react";
              ...
              {["All", "Onboarding", "Offboarding", "Appraisal", "Statutory", "Other"].map(cat => (
                <button 
                  ...
                  className={`w-full text-left px-3 py-2 text-sm rounded-lg transition-colors flex items-center justify-between group ${
                    activeCategory === cat ? "bg-secondary text-foreground font-semibold" : "text-muted-foreground hover:bg-secondary/50 hover:text-foreground"
                  }`}
                >
              ...
              <div key={i} className="bg-card border border-border rounded-xl p-5 hover:shadow-md transition-all group border-l-4 border-l-primary/30">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button className="p-1.5 hover:bg-secondary rounded-lg text-muted-foreground" title="Edit"><Edit3 className="w-3.5 h-3.5" /></button>
                    <button className="p-1.5 hover:bg-secondary rounded-lg text-muted-foreground" title="Delete"><Trash2 className="w-3.5 h-3.5 hover:text-rose-500 transition-colors" /></button>
                  </div>
                </div>
