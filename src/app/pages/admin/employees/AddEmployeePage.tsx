import { useState, useEffect, useCallback, useRef, type ReactNode, type InputHTMLAttributes, type SelectHTMLAttributes, type ElementType, type FormEvent } from 'react';
import { useNavigate } from 'react-router';
import AddEmployeeBasicForm from '@/app/components/employees/AddEmployeeBasicForm';

import {
  User, Briefcase, Clock, CreditCard, Calendar, FileText, Shield,
  Search, Mail, Hash, Phone, MapPin, UserPlus, Building2, RefreshCw,
  Eye, EyeOff, Upload, X, Check, AlertCircle, CheckCircle, ChevronDown,
  Loader2, ArrowLeft, Save, Info, ExternalLink, MoreHorizontal, HelpCircle
} from 'lucide-react';
...
function Radio({ opts, val, set, name }: { opts: { v: string; l: string }[]; val: string; set: (v: string) => void; name: string }) {
  return (
    <div className="flex items-center gap-6 h-9">
      {opts.map(o => (
        <label key={o.v} className="flex items-center gap-2.5 cursor-pointer group">
          <div className="relative flex items-center justify-center">
            <input type="radio" name={name} value={o.v} checked={val === o.v} onChange={() => set(o.v)} className="sr-only" />
            <div className={`w-4 h-4 rounded-full border transition-all duration-150 ${val === o.v ? "border-foreground bg-foreground shadow-[0_0_0_3px_rgba(0,0,0,0.05)]" : "border-border bg-card group-hover:border-muted-foreground"}`} />
...
function CheckInput({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <label className="flex items-center gap-3 p-3 rounded-lg border border-border bg-secondary/20 hover:bg-secondary/40 transition-colors cursor-pointer group">
      <div className="relative flex items-center justify-center">
        ...
        <div className={`w-5 h-5 rounded border transition-all duration-150 ${checked ? "bg-foreground border-foreground" : "border-border bg-card group-hover:border-muted-foreground"}`} />
        {checked && <CheckCircle size={14} className="text-primary-foreground absolute" strokeWidth={3} />}
      </div>
...
export function AddEmployeePage() {
  ...
  useEffect(() => {
    const u = form.firstName.toLowerCase().replace(/\s+/g, ".").replace(/[^a-z0-9.]/g, "");
  ...
  const vField = useCallback((k: keyof FormState): boolean => {
    const REQ: (keyof FormState)[] = ["firstName", "email", "department", "designation", "employmentType", "joiningDate", "aadhaarNumber", "reportingManager"];
    ...
    if (k === "aadhaarNumber" && form.aadhaarNumber && !/^\d{12}$/.test(form.aadhaarNumber.replace(/\s/g, ""))) {
      setErrors(e => ({ ...e, aadhaarNumber: "Must be 12 digits" })); return false;
    }
  ...
  const validateAll = (): boolean => {
    ...
    if (form.aadhaarNumber && !/^\d{12}$/.test(form.aadhaarNumber.replace(/\s/g, ""))) {
      errs.aadhaarNumber = "Must be 12 digits"; ok = false;
    }
    setErrors(errs);
        ...
        <div className="flex gap-8 items-start">
          ...
          <form id="add-employee-form" onSubmit={handleSubmit} noValidate
            ...
            <SC id="s-basic" n={1} title="Basic Information"
              desc="Personal details, employment setup, and contact information" Icon={User}>

              {/* LEFT SIDE */}
              <div className="flex flex-col gap-5">
                <FF label="Employee Number Series" required>
                  <div className="space-y-1.5">
                    <Sel value={form.employeeSeries} onChange={e => set("employeeSeries", e.target.value)}
                      ph="Select series" opts={SERIES} />
                    <button type="button" className="text-[10px] font-bold text-foreground hover:underline flex items-center gap-1 uppercase tracking-wider">
                ...
                <FF label="Employee No" required hint="Based on selected series">
                  <Inp value={form.employeeId} onChange={e => set("employeeId", e.target.value)} 
                    icon={<Hash size={13} />} placeholder="EMP-001" />
                </FF>

                <FF label="Name" required error={errors.firstName} ok={ok("firstName")}>
                  <Inp name="firstName" value={form.firstName}
                    onChange={e => set("firstName", e.target.value)} onBlur={() => blur("firstName")}
                    placeholder="Full Name" icon={<User size={13} />} err={!!errors.firstName} success={ok("firstName")} />
                </FF>
                ...
                <FF label="Aadhaar Number" required>
                  <Inp value={form.aadhaarNumber} onChange={e => set("aadhaarNumber", e.target.value)}
                    placeholder="XXXX XXXX XXXX" icon={<FileText size={13} />} />
                </FF>

                <FF label="Gender" required>
                  <Radio opts={GENDER_OPTS} val={form.gender} set={v => set("gender", v)} name="gender" />
                </FF>
                ...
                <FF label="Date of Joining" required error={errors.joiningDate} ok={ok("joiningDate")}>
                  <Inp type="date" value={form.joiningDate}
                    onChange={e => set("joiningDate", e.target.value)} onBlur={() => blur("joiningDate")}
                    err={!!errors.joiningDate} success={ok("joiningDate")} className="cursor-pointer" />
                </FF>

                <FF label="Referred By">
                  <Sel value={form.referredBy} onChange={e => set("referredBy", e.target.value)}
                    ph="Select referral"
                    opts={MANAGERS.map(m => ({ v: m.v, l: m.l }))} />
                </FF>

                <div className="pt-2 flex flex-col gap-3">
                  <CheckInput label="Allow the employee to fill in their information" 
                    checked={form.allowSelfFill} onChange={v => set("allowSelfFill", v)} />
                  <button type="button" className="text-[10px] font-bold text-foreground hover:underline flex items-center gap-1 uppercase tracking-wider w-fit">
              ...
              <div className="flex flex-col gap-5">
                <FF label="Probation Period" hint="Specify duration in days">
                  <div className="flex items-center gap-2">
                    <Inp type="number" value={form.probationPeriod} 
                      onChange={e => set("probationPeriod", e.target.value)}
                      placeholder="90" className="flex-1" />
                ...
                <FF label="Confirmation Date" hint="Auto-calculated from Joining Date">
                  <Inp type="date" value={form.confirmationDate} onChange={e => set("confirmationDate", e.target.value)} 
                    className="cursor-pointer bg-secondary/50" readOnly />
                </FF>
                ...
                <FF label="Emergency Contact Name">
                  <Inp value={form.emergencyName} onChange={e => set("emergencyName", e.target.value)}
                    placeholder="Contact Name" icon={<UserPlus size={13} />} />
                </FF>

                <FF label="Emergency Contact Number">
                  <Inp value={form.emergencyPhone} onChange={e => set("emergencyPhone", e.target.value)}
                    placeholder="Contact Number" icon={<Phone size={13} />} />
                </FF>

                <FF label="Father's Name">
                  <Inp value={form.fathersName} onChange={e => set("fathersName", e.target.value)}
                    placeholder="Full Name" icon={<User size={13} />} />
                </FF>

                <FF label="Spouse Name">
                  <Inp value={form.spouseName} onChange={e => set("spouseName", e.target.value)}
                    placeholder="Full Name" icon={<User size={13} />} />
                </FF>
