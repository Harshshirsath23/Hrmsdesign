...
import { Check, ChevronDown, ChevronRight, Download, Eye, FileText, Paperclip, X } from "lucide-react";
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">
              Employee: <span className="font-semibold">{getEmployeeDisplayName(request.employee_id)}</span>
              {" "}({request.employee_id}) • {new Date(request.created_at).toLocaleString("en-IN")}
            </p>
          </div>
        </div>
...
export function ProfileChangeRequestsPage() {
  ...
  const allRequests = useMemo(() => getChangeRequests(), [refreshTick]);

  const requests = useMemo(() => {
    // Exclude draft status from admin view
    const noDraft = allRequests.filter((r) => r.status !== "draft");
    if (statusFilter === "all") return noDraft;
    return noDraft.filter((r) => r.status === statusFilter);
  }, [allRequests, statusFilter]);

  const counts = useMemo(() => ({
    all: allRequests.filter((r) => r.status !== "draft").length,
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        ...
        {/* Filter tabs */}
        <div className="flex flex-wrap items-center gap-2 mt-4 border-t border-border pt-4">
      ...
      {requests.length === 0 && (
        <div className="rounded-xl border border-dashed border-border bg-card p-12 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">No profile change requests match the selected filter.</p>
        </div>
      )}
